...
- Full user journey: sync-profile -> onboarding -> search -> facility
  detail -> save/unsave (idempotent double-save) -> inquiry -> assessment
- Guest flow: can browse/search/onboard/take the assessment, but submitting
  an inquiry or saving/liking a facility requires a real login (guest token
  is rejected on those two) -- and cannot access `/profile/me`
- 404/400 handling for missing/invalid facility ids
