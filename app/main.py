...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        # Client is still connected but sent nothing within the window --
        # the socket is still open, so closing it is correct and safe.
        log_ws(f"Client sent nothing before timeout │ session={session_id}")
        try:
            await websocket.close(code=1008)
        except RuntimeError:
            pass  # already closed underneath us -- nothing to do
        return
    except WebSocketDisconnect:
        # Client already closed the socket before sending its first frame
        # (normal when backing out of the screen or a fast cold-start socket
        # swap). There's nothing left to close -- attempting to would raise
        # "Unexpected ASGI message 'websocket.close'". Just return.
        log_ws(f"Client disconnected before first frame │ session={session_id}")
        return
    except ValueError:
        # Malformed JSON in the first frame -- the socket is still open, so a
        # close is valid, but guard it in case the client raced us to it.
        log_ws(f"Malformed first frame │ session={session_id}")
        try:
            await websocket.close(code=1003)
        except RuntimeError:
