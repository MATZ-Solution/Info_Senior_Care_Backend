...
class Facility(Base):
    ...
    # External identity (CMS Certification Number). Nullable; uniqueness
    # enforced by a partial index (see migration) so multiple NULLs are allowed.
    ccn: Mapped[str | None] = mapped_column(String(20), nullable=True)

    # Our own dedup key, ALWAYS computed (see services/dedup.py); fallback match
    # key when ccn is absent. Makes future file imports idempotent.
    dedup_hash: Mapped[str] = mapped_column(String(64), nullable=False)

    # The SOURCE file's own row identifier (its "uuid" column). Most reliable
    # match key across re-imports; kept separate from our own `id`.
    source_uuid: Mapped[str | None] = mapped_column(String(64), index=True)

    name: Mapped[str] = mapped_column(String(500), nullable=False)
    facility_type: Mapped[str | None] = mapped_column(String(200), index=True)
    # Standardized bucket the frontend filter uses. Constrained at the DB level
    # to the 8 approved categories (see CHECK ck_facilities_facility_type_category,
    # migration c7f2a9d4e8b1).
    facility_type_category: Mapped[str | None] = mapped_column(String(100), index=True)
    ownership_type: Mapped[str | None] = mapped_column(String(200))
    ...
    facility_subtype: Mapped[str | None] = mapped_column(String(200))
    # NPI enumeration type from the source (e.g. "1" individual / "2" org).
    npi_type: Mapped[str | None] = mapped_column(String(20))

    latitude: Mapped[float | None] = mapped_column(Float)
    longitude: Mapped[float | None] = mapped_column(Float)

    bed_count: Mapped[int | None] = mapped_column(Integer)
    # Secure memory-care bed capacity; distinct from general bed_count.
    secure_memory_care_beds: Mapped[int | None] = mapped_column(Integer)
    overall_rating: Mapped[float | None] = mapped_column(Float)
    ...
    # Partial unique index on ccn and unique index on dedup_hash are created in
    # the Alembic migration (raw SQL), not here.
    __table_args__ = (
        Index("ix_facilities_state_type", "state", "facility_type"),
    )
