...
# The normalized, canonical table. `facilities.id` is the ONLY id the rest of
# the app ever uses (list endpoints, /facilities/{facility_id} detail lookup,
# saved facilities, inquiries...). It is generated fresh at import time
# (uuid4()) and is deliberately NOT the same value as this source table's own
# `uuid` column (see scripts/import_facilities.py) -- so the index's document
# id must be sourced from here, not from the raw ingestion row, or a card's
# id and the id you get back from opening that card point at two different
# UUID spaces (search dutifully returns a card, the detail lookup 404s, or --
# worse, if a value happens to collide -- returns a completely different
# facility).
...
def build_select_sql() -> str:
    ...
    JOINED against `facilities`, not selected from the source table alone:
    the document's `uuid` (-> TypeSense `id`) comes from `facilities.id`
    (`fac.id`), the canonical primary key every other endpoint uses --
    NOT from this source table's own `"uuid"` column, which is a different,
    unrelated id generated by the original CMS/state export. An INNER JOIN
    also means a source row that hasn't been imported into `facilities` yet
    (or whose facility was deactivated) is correctly left out of the index,
    since there would be nothing for a search result to link to anyway.
    """
    ...
    return (  # noqa: S608 — identifiers are hardcoded; values are bound params
        ...
        f'  AND (CAST(:after_uuid AS text) IS NULL OR "uuid" > CAST(:after_uuid AS text)) '
        f'  AND (CAST(:since AS timestamp) IS NULL OR src."updated_at" > CAST(:since AS timestamp)) '
        f'ORDER BY "uuid" '
        f"LIMIT :limit"
    )
...
def build_all_ids_sql() -> str:
    ...
    Returns `facilities.id` (same join as build_select_sql), matching what
    is actually stored as each document's `id` in TypeSense — comparing
    against the source table's own `"uuid"` here would make every single
    document look "stale" and get deleted on the next reconciliation run.
    """
    return (  # noqa: S608 — see build_select_sql
        f"SELECT fac.id::text "
        f"FROM {SOURCE_TABLE} AS src "
        f"JOIN {FACILITIES_TABLE} AS fac ON fac.source_uuid = src.\"uuid\" "
        f"WHERE fac.is_active = true"
    )